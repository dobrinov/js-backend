...
async function decodeJWT(token: string) {
  ...
  if (!('userId' in claims) || typeof claims.userId !== 'number') return {error: 'Invalid token'}

  const user = await prisma.user.findUnique({where: {id: claims.userId}})

  if (!user) return {error: 'Cannot find user'}

  if (!('exp' in claims) || typeof claims.exp !== 'number') return {error: 'Invalid token'}

  if (claims.exp * 1000 < Date.now()) return {error: 'Token expired'}

  const shadowingUserId =
    'shadowingUserId' in claims && typeof claims.shadowingUserId === 'number' ? claims.shadowingUserId : undefined
...
async function execute() {
  ...
  server.post<{
    ...
    if (!authorization) return response.code(401).send(errorResponseBody('Unauthorized'))

    const token = authorization.replace('Bearer ', '')
    const {user, error} = await decodeJWT(token)

    if (error || !user) return response.code(401).send(errorResponseBody(error ?? 'Unauthorized'))
    ...
    try {
      ...
      let message = error instanceof Error ? error.message : 'Oops something went wrong'
  ...
  server.post<{Body: {email: string; password: string}}>('/session', async function (request, response) {
    ...
    if (!checkPassword(password, user.passwordDigest)) return response.code(401).send('Invalid email or password')
  ...
  server.post<{Body: {userId: string}}>('/impersonate', async function (request, response) {
    const authorization = request.headers.authorization
    ...
    if (!user) return response.code(401).send(errorResponseBody('Unauthorized'))
    ...
    if (!userToSuspend) return response.code(400).send(errorResponseBody('User not found'))

    if (userToSuspend.id === user.id) return response.code(400).send(errorResponseBody('Cannot impersonate yourself'))

    if (userToSuspend.suspendedAt !== null) {
      return response.code(400).send(errorResponseBody('Cannot impersonate a suspended user'))
    }
  ...
  server.delete('/unimpersonate', async function (request, response) {
    ...
    if (!shadowingUserId) return response.code(401).send(errorResponseBody('Not in impersonation session'))
    if (error) return response.code(401).send(errorResponseBody(error))
